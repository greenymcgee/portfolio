...
- [ ] Unused parameters in callbacks are named `_` — the one exception to the no-single-letter-names rule.
...
This uses the clean-authoring exception that allows `_` for unused callback
parameters.
...
### 4. Decompose complex computed properties into named getters

When a class method or getter orchestrates several sub-computations, extract
each sub-step as a private getter with a domain name. The public surface stays
at a single level of abstraction; each private getter owns exactly one step.

Inlined (avoid):

```ts
get availablePages() {
  const pageRangeStart = Math.max(0, Math.min(this.currentPage - 1, this.totalPages - 3))
  const pageRangeEnd = Math.min(this.totalPages - 1, Math.max(this.currentPage + 1, 2))
  const hasLeadingGap = pageRangeStart > 1
  const pageRange = Array.from(
    Array(pageRangeEnd - pageRangeStart + 1),
    (_, index) => pageRangeStart + index + 1,
  ).filter((page) => page !== 1)
  return [1, ...(hasLeadingGap ? ['ellipsis'] : []), ...pageRange, ...]
}
```
...
Each getter is a single expression with a domain name. The public getter reads
as a declarative description of the output; nothing needs to be traced to
understand its shape.

### 5. Precise types over escape hatches

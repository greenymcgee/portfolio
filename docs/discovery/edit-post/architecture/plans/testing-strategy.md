...
| Subject | Cases |
|---------|-------|
| `EditPostForm` autosave | Debounce fires after 1s; `cancelDebounce` prevents fire; state transitions (idle → saving → saved → error) |
| `AutoSaveStatus` | All four states render correctly (`idle`, `saving`, `saved`, `error`) |

...
## 2026-05-30 - D52: `SiteNavbar` stays mounted in the root layout, self-guarded by a route allowlist (supersedes D47)

- **Decision:** Reverse D47. `SiteNavbar` and `ClientSiteNavbar` remain mounted in the root `app/layout.tsx` exactly as before — neither is removed and `ClientSiteNavbar`/`usePathname` are retained. Route-level visibility is enforced by a single guard inside `SiteNavbar`: `if (pathname !== ROUTES.home && pathname !== ROUTES.posts) return null`. The navbar therefore renders only on `/` and `/posts`, and returns `null` everywhere else (`/login`, `/posts/[id]`, `/posts/[id]/edit`, and any future route). `AdminMenu` continues to render inside `SiteNavbar`, so post pages that need the admin menu reach it through their own page-level rendering, not the navbar.
- **Why:** The opt-in-at-the-call-site model (D47) remounts `SiteNavbar` on every navigation, because each page renders its own instance. The Work↔Blog sliding-pill indicator is a CSS `transition-all` on an absolutely-positioned element — it only animates when the element stays mounted across the route change. Remounting snaps it into place instead of sliding. Keeping the navbar mounted in the root layout preserves that animation, which was the whole point of the component. The original objection to a per-path conditional (D47's "buried-list problem") was specifically about a *denylist* — an exclusion list that every new route must remember to update. This guard is an *allowlist*: it names the two routes that *should* show the navbar, stays in one place inside `SiteNavbar`, and a new route is hidden by default with no edit required. The bounded allowlist sidesteps the maintenance concern that motivated D47 while keeping the animation.
- **Alternatives considered:** D47's opt-in call-site rendering — rejected here because it sacrifices the slider animation. Route-group layouts that mount the navbar only for `/` and `/posts` — preserves opt-in but reintroduces the remount (separate layout subtrees) and adds file restructuring for the same visual regression. A shared layout segment wrapping only the two animated routes — same remount problem unless the two routes share one persistent layout, which the current route tree does not.

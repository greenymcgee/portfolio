...
- **Templates resolved** (2026-04-25, separate session). The SKILL templates that previously did not exist are now seeded at `.cursor/skills/architecture-discovery/templates/` (inputs, tracking files, `architecture.md`, `refinement-checklist.md`, `review-summary.md`, and a full `plans/` skeleton). The `fix-register-page` artifacts were the source seed — when a template feels thin, read the corresponding artifact in this discovery for the filled-in shape. `templates/README.md` is the orientation entry.
